...
- 2026-06-23：阶段 17 远端测试兼容性修复已整合：搜索内容筛选测试不再依赖集合顺序，音频/视频素材测试改用 UTType conforms 判断，视频缩略图缓存 key 测试通过取帧时间验证差异，`MemoBackupPackage.fileExtension` 改为 `nonisolated` 以避开纯常量的 actor 隔离误报，衣橱天气洞察保留含标点的原始天气短语。本地验证通过：`git diff --check`、`plutil -lint`、旧 Swift parser 覆盖 `MemoSearchQuery.swift` / `VideoThumbnailGenerator.swift` / `WardrobeInsightEngine.swift` / `SomeTests.swift`；最新 GitHub Actions run `28009254154` 仍在 Run tests，需推送后继续复验。
