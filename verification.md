...
- 2026-06-24：阶段 77 App Shortcuts 打开常用工作区本地接线验证：`testAppShortcutRouteStoreOpensAndConsumesDestination` 覆盖 App Group UserDefaults 路由请求、切换到工作日志、清空旧搜索和只消费一次；`rg -n "OpenZenIntent|OpenWorkLogIntent|OpenWardrobeIntent|OpenAIWorkspaceIntent|AppShortcutRouteStore|openPendingShortcutDestinationIfNeeded|打开专注|打开工作日志|打开衣橱|打开 AI" some/AppIntents some/Stores/MemoStore.swift some/SomeApp.swift SomeTests/SomeTests.swift README.md` 确认 App Shortcuts、启动/激活消费和文档接线；另通过 `xcrun swiftc -parse some/Stores/MemoStore.swift SomeTests/SomeTests.swift`、`git diff --check`、plist/scheme/workflow 校验。本机只有 Command Line Tools，`xcodebuild -version` 报 active developer directory 不是完整 Xcode，`gh` 不存在；App Intents 源在旧 Swift parser 下会被 async/AppIntents 语法拦截，完整编译和远端 CI 状态以 GitHub Actions/Xcode 16 为准。
