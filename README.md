...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图 OCR v2、素材索引、素材库视图、手帐页面与画布编辑 v5、工作日志 v1、图片编辑 v5、电子衣橱与穿着洞察 v5、分享扩展、引用、备份、隐私锁和可选 AI 洞察；物体抠图、真实天气 API 和行程天数等仍在路线图中。
...
- 图片编辑 v5：素材库图片可打开编辑器，支持预设比例裁剪、裁剪中心/缩放微调、背景柔化/纯色画布、人物抠图、用户拥有或获授权图片的瑕疵清理贴片、滤镜、边框、文字、贴纸和版式模板/导出预设；保存为新的本地 PNG 附件与“图片编辑”素材，并记录所用模板便于复现
...
- 统一素材索引 v1：现有记录会同步索引为文本、链接、网页摘录、摘录片段、图片文字、图片编辑、附件、音频、视频、任务、内部引用、手帐页面、工作日志、衣橱单品、穿搭、穿着记录、洗护记录和打包清单素材，为后续人物/物体抠图、真实天气推荐和衣橱统计打底
