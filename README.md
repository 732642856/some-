...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、相册/文件导入、素材索引、素材库视图、分享扩展、引用、备份、隐私锁和可选 AI 洞察；手帐排版、图片编辑、网页提炼、录音/拍摄、电子衣橱等是新的产品目标，已写入路线图但尚未完整实现。
...
- 标签筛选
- SQLite FTS 全文搜索
- 搜索语法：`#标签`、`tag:标签`、`is:pinned`、`is:archived`、`is:active`、`has:link`、`created:2026-06`
...
- 统一素材索引 v1：现有记录会同步索引为文本、链接、附件、任务和内部引用素材，为后续网页摘录、手帐、图片编辑和电子衣橱打底
- 素材库 v1：首页可按文本、链接、附件、任务、引用等类型筛选素材，附件图片支持缩略图预览并可跳回来源记录
