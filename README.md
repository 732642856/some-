...
- AI 自然语言语义搜索：未配置 Key 时走本地带权相关搜索并显示命中词，配置 Key 后走 OpenAI embedding，并在本次运行中缓存记录 embedding 以减少重复请求
- AI 相关记录查找：未配置 Key 时可用本地带权相关度和命中词解释，配置 Key 后走 OpenAI embedding，并复用同一批记录 embedding
