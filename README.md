...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图 v1、网页摘录 v1、图片/截图 OCR v1、素材索引、素材库视图、手帐页面与画布编辑 v1、工作日志 v1、图片编辑 v2、电子衣橱与衣橱洞察 v1、分享扩展、引用、备份、隐私锁和可选 AI 洞察；完整手势裁剪器、抠图、穿着日历、成本/次统计和天气推荐等仍在路线图中。
...
- 后续补连续扫描增强、转写语言选择、缩略图缓存和批量媒体元数据
...
- 视频缩略图 v1：详情附件区和素材库视频条目会在本机生成视频画面预览，失败时回退为视频图标
...
当前版本不接入账号、云同步、广告或第三方分析。笔记内容默认以 SQLite 保存在用户设备本地；启用 App Group 后，主 App 与 Share Extension 会共用同一个本地 SQLite 数据库。Markdown 导出和完整备份导出由用户主动触发系统分享表。视频缩略图通过 Apple AVFoundation 在本机按需生成，当前不上传原视频，也暂不持久化缓存缩略图文件。
