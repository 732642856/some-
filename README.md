...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图 OCR v1、素材索引、素材库视图、手帐页面与画布编辑 v5、工作日志 v1、图片编辑 v4、电子衣橱与穿着洞察 v4、分享扩展、引用、备份、隐私锁和可选 AI 洞察；完整人物/物体抠图、真实天气 API、行程天数和洗护本地通知等仍在路线图中。
...
- 图片/截图 OCR v2：拍摄或导入图片后用 Apple Vision 在本机识别文字，识别结果随附件保存为“图片文字”记录；底层已支持按归一化区域裁切后识别并记录区域信息，后续可接完整框选 UI
