...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v2、图片/截图 OCR v1、素材索引、素材库视图、手帐页面与画布编辑 v1、工作日志 v1、图片编辑 v3、电子衣橱与穿着洞察 v2、分享扩展、引用、备份、隐私锁和可选 AI 洞察；完整人物/物体抠图、天气推荐和旅行打包等仍在路线图中。
...
- 电子衣橱 v2：首页新增“衣橱”模式，可快速保存单品、价格、穿搭组合和穿着记录，单品可绑定已有图片附件，并进入衣橱/穿搭/穿着素材索引；衣橱洞察会统计分类、颜色、季节、场景、未搭配单品、常用单品、最近穿着和成本/次，并可把建议填入穿搭草稿
- `#标签` 自动识别
- 支持 `#项目/子类` 多级标签
- 标签筛选
- SQLite FTS 全文搜索
- 搜索语法：`#标签`、`tag:标签`、`is:pinned`、`is:archived`、`is:active`、`has:link`、`has:web`、`has:ocr`、`has:image-edit`、`has:audio`、`has:video`、`has:scrapbook`、`has:worklog`、`has:wardrobe`、`has:outfit`、`has:wear-log`、`created:2026-06`
...
- 统一素材索引 v1：现有记录会同步索引为文本、链接、网页摘录、图片文字、图片编辑、附件、音频、视频、任务、内部引用、手帐页面、工作日志、衣橱单品、穿搭和穿着记录素材，为后续人物/物体抠图、旅行打包和衣橱统计打底
