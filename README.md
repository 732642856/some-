...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图 OCR v2、素材索引、素材库视图、手帐页面与画布编辑 v5、工作日志 v1、图片编辑 v6、电子衣橱与穿着洞察 v5、分享扩展、引用、备份、隐私锁和可选 AI 洞察；真实天气 API、行程天数和单实例主体选择等仍在路线图中。
...
- 电子衣橱 v5：首页新增“衣橱”模式，可快速保存单品、价格、穿搭组合、穿着记录、洗护状态和旅行打包清单，单品可绑定已有图片附件，并进入衣橱/穿搭/穿着/洗护/打包素材索引；衣橱洞察会统计分类、颜色、季节、场景、未搭配单品、常用单品、最近穿着和成本/次，根据最近穿着天气生成天气穿搭，避开待清洗/送洗/待熨烫/待修补单品，提醒洗护状态，可把穿搭建议或自动打包建议填入对应草稿，并可为待处理洗护单品安排本地系统通知
- `#标签` 自动识别
- 支持 `#项目/子类` 多级标签
- 标签筛选
- SQLite FTS 全文搜索
