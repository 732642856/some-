...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v2、网页/OCR 摘录片段索引 v1、图片/截图 OCR v1、素材索引、素材库视图、手帐页面与画布编辑 v5、工作日志 v1、图片编辑 v4、电子衣橱与穿着洞察 v4、分享扩展、引用、备份、隐私锁和可选 AI 洞察；完整人物/物体抠图、真实天气 API、行程天数和洗护本地通知等仍在路线图中。
...
- 网页摘录入口 v2：输入卡片识别到链接后可提取标题、摘要、来源、摘录卡和正文重点候选；若草稿里已有 OCR 文本，也会合并为可勾选摘录片段后再保存，并作为“摘录”素材进入素材库与搜索
...
- 工作日志 v1：首页新增“日志”模式，可勾选已有记录，提取已完成/未完成任务，生成带来源引用的日报、周报或项目日志
...
Share Extension 对网页标题的支持依赖系统分享项提供的标题/摘要；主 App 快速输入里的网页摘录会主动抓取网页 HTML 的标题、description、正文段落和摘录卡信息，过滤脚本、导航、页脚、登录/订阅提示等常见噪音，并允许在保存前勾选网页/OCR 片段，失败时回退系统链接元数据或原 URL。当前仍不是完整 Readability 正文提取，也不会绕过登录、付费墙或网站访问限制。
