...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v2、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图 OCR v3、素材索引、素材库视图、手帐页面与画布编辑 v6、工作日志 v7、图片编辑 v9、电子衣橱与穿着洞察 v7、分享扩展、引用、备份、隐私锁和可选 AI 洞察；真实天气 API 和复杂对象修复等仍在路线图中。
...
- 电子手帐 v6：首页新增“手帐”模式，可保存手帐页面结构化记录、生成可迁移图层 JSON，在手帐列表显示页面预览，可在编辑器拖拽、缩放、旋转图层，追加已有图片素材，调整画布底色、字体、文字色、贴纸、底色、花边颜色、字号、圆角和线宽，把当前画布导出为 PNG 或 PDF 附件并立即调起系统分享表，也可选择多张图片快速生成图片拼贴和 PNG 导出附件
