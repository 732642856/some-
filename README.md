...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、禅定专注记录、小组件快照 v1、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v2、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图 OCR v3、素材索引、素材库视图、手帐页面与画布编辑 v8、工作日志 v7、图片编辑 v9、电子衣橱与穿着洞察 v7、分享扩展、引用、备份 v2、隐私锁和可选 AI 洞察；真实天气 API 和复杂对象修复等仍在路线图中。
...
- 小组件快照 v1：主 App 保存/删除记录时写入最近记录快照，小组件展示今日/全部计数、最近记录，并可通过 `some://home`、`some://zen`、`some://open?id=...` 回到 App
