...
- 网页摘录入口 v2：输入卡片识别到链接后可一键保存标题、摘要、来源、摘录卡和正文重点候选；本地 HTML 清洗会过滤脚本、导航、页脚和常见噪音段落
...
Share Extension 对网页标题的支持依赖系统分享项提供的标题/摘要；主 App 快速输入里的网页摘录会主动抓取网页 HTML 的标题、description、正文段落和摘录卡信息，过滤脚本、导航、页脚、登录/订阅提示等常见噪音，失败时回退系统链接元数据或原 URL。当前仍不是完整 Readability 正文提取，也不会绕过登录、付费墙或网站访问限制。
