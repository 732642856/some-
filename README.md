...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、禅定专注记录、桌面小组件快速入口、标签、搜索、附件、拍照/拍视频/连续扫描/相册/文件导入、录音 v1、本地语音转写 v2、视频缩略图缓存与媒体元数据摘要 v2、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图/扫描 OCR v4、素材索引、素材库视图、手帐页面与画布编辑 v8、工作日志 v7、图片编辑 v9、电子衣橱与穿着洞察 v7、分享扩展、引用、备份 v2、隐私锁和本地/可选联网 AI 工作台；真实天气 API 和复杂对象修复等仍在路线图中。
...
- 工作日志 v7：首页“日志”模式可按标签、素材类型、时间和关键词筛选可汇总记录，勾选已有记录，提取已完成/未完成任务，填写项目与日期范围，套用日报、周报、项目汇报或复盘模板，生成带来源引用的结构化工作记录；工作日志列表可按项目/模板/日期继续筛选，并把当前结果导出为带本地汇报摘要的 Markdown、结构化 CSV 或可直接发送的纯文本汇报稿
...
- AI 自然语言语义搜索：未配置 Key 时走本地带权相关搜索并显示命中词，配置 Key 后走 OpenAI embedding
- AI 相关记录查找：未配置 Key 时可用本地带权相关度和命中词解释，配置 Key 后走 OpenAI embedding
