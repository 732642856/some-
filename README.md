...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、相册/文件导入、网页摘录 v1、图片/截图 OCR v1、素材索引、素材库视图、分享扩展、引用、备份、隐私锁和可选 AI 洞察；手帐排版、图片编辑、录音/拍摄、电子衣橱等是新的产品目标，已写入路线图但尚未完整实现。
...
- 随时记录想法、摘录、待办、工作记录
- 拍摄或导入图片、截图、文件；导入图片时会尝试本地 OCR 提取文字
- 后续补录音、视频拍摄、扫描增强、语音转写
...
- 快速输入卡片
- 快速导入入口 v1：可从输入卡片导入相册图片/视频和文件，保存为本地附件素材
- 网页摘录入口 v1：输入卡片识别到链接后可一键保存标题、摘要和重点候选
- 图片/截图 OCR v1：导入图片后用 Apple Vision 在本机识别文字，识别结果随附件保存为“图片文字”记录
- `#标签` 自动识别
- 支持 `#项目/子类` 多级标签
- 标签筛选
- SQLite FTS 全文搜索
- 搜索语法：`#标签`、`tag:标签`、`is:pinned`、`is:archived`、`is:active`、`has:link`、`has:web`、`has:ocr`、`created:2026-06`
...
- 统一素材索引 v1：现有记录会同步索引为文本、链接、网页摘录、图片文字、附件、任务和内部引用素材，为后续手帐、图片编辑和电子衣橱打底
- 素材库 v1：首页可按文本、链接、网页、图片文字、附件、任务、引用等类型筛选素材，附件图片支持缩略图预览，网页素材可直接打开原链接
...
图片 OCR 当前使用 Apple Vision `VNRecognizeTextRequest` 在设备本地处理导入图片，不需要把图片上传到第三方服务。没有识别出文字时仍会按普通图片附件保存；相机扫描、批量 OCR、版面分区和置信度展示仍属于后续阶段。
