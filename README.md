...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v2、图片/截图 OCR v1、素材索引、素材库视图、手帐页面与画布编辑 v5、工作日志 v1、图片编辑 v4、电子衣橱与穿着洞察 v4、分享扩展、引用、备份、隐私锁和可选 AI 洞察；完整人物/物体抠图、真实天气 API、行程天数和洗护本地通知等仍在路线图中。
...
- 网页摘录入口 v2：输入卡片识别到链接后可提取标题、摘要、来源、摘录卡和正文重点候选；若草稿里已有 OCR 文本，也会合并为可勾选摘录片段后再保存
...
- 标签筛选
- SQLite FTS 全文搜索
- 搜索语法：`#标签`、`tag:标签`、`is:pinned`、`is:archived`、`is:active`、`has:link`、`has:web`、`has:ocr`、`has:image-edit`、`has:audio`、`has:video`、`has:scrapbook`、`has:worklog`、`has:wardrobe`、`has:outfit`、`has:wear-log`、`has:laundry-log`、`has:packing-list`、`created:2026-06`
...
- 统一素材索引 v1：现有记录会同步索引为文本、链接、网页摘录、图片文字、图片编辑、附件、音频、视频、任务、内部引用、手帐页面、工作日志、衣橱单品、穿搭、穿着记录、洗护记录和打包清单素材，为后续人物/物体抠图、真实天气推荐和衣橱统计打底
- 素材库 v1：首页可按文本、链接、网页、图片文字、图片编辑、附件、音频、视频、任务、引用、手帐、日志、衣橱、穿搭等类型筛选素材，图片与视频附件支持本地预览，网页素材可直接打开原链接，图片素材可进入编辑器
