...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图 OCR v2、素材索引、素材库视图、手帐页面与画布编辑 v5、工作日志 v6、图片编辑 v9、电子衣橱与穿着洞察 v7、分享扩展、引用、备份、隐私锁和可选 AI 洞察；真实天气 API 和复杂对象修复等仍在路线图中。
...
- 图片编辑 v9：素材库图片可打开编辑器，支持预设比例裁剪、拖拽定位/捏合缩放裁剪、背景柔化/纯色画布、人物抠图、iOS 17+ 智能主体抠图、点选单个智能主体、用户拥有或获授权图片的柔和修补和对象清理贴片、滤镜、边框、文字、贴纸和版式模板/导出预设；裁剪预览与导出渲染共用同一套边界规则，保存为新的本地 PNG 附件与“图片编辑”素材，并记录所用模板、主体选择点和清理类型便于复现
