...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、禅定专注记录、桌面小组件快速入口、标签、搜索、附件、拍照/拍视频/连续扫描/相册/文件导入、录音 v1、本地语音转写 v2、视频缩略图缓存与媒体元数据摘要 v2、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图/扫描 OCR v4、素材索引、素材库视图、手帐页面与画布编辑 v8、工作日志 v9、图片编辑 v10、电子衣橱与穿着洞察 v9、分享扩展、引用、备份 v2、隐私锁和本地/可选联网 AI 工作台；复杂对象修复等仍在路线图中。
...
- 图片/截图/扫描 OCR v5：拍摄、连续扫描或导入图片后用 Apple Vision 在本机识别文字，识别结果随附件保存为“图片文字/扫描文字”记录，并记录平均/最低识别置信度；详情页可对图片附件框选局部区域并追加识别文字，多张图片、多页扫描或多段局部 OCR 会分别进入“图片文字”素材和 `has:ocr` 搜索
