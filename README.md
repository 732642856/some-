...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v2、图片/截图 OCR v1、素材索引、素材库视图、手帐页面与画布编辑 v1、工作日志 v1、图片编辑 v2、电子衣橱与衣橱洞察 v1、分享扩展、引用、备份、隐私锁和可选 AI 洞察；完整手势裁剪器、抠图、穿着日历、成本/次统计和天气推荐等仍在路线图中。
...
- 图片编辑 v2：素材库图片可打开编辑器，支持预设比例裁剪、裁剪中心/缩放微调、用户拥有或获授权图片的瑕疵清理贴片、滤镜、边框、文字和贴纸，保存为新的本地 PNG 附件与“图片编辑”素材
...
- 电子衣橱 v1：首页新增“衣橱”模式，可快速保存单品和穿搭组合，单品可绑定已有图片附件，并进入衣橱/穿搭素材索引；衣橱洞察会统计分类、颜色、季节、场景、未搭配单品和常用单品，并可把建议填入穿搭草稿
- `#标签` 自动识别
- 支持 `#项目/子类` 多级标签
- 标签筛选
- SQLite FTS 全文搜索
- 搜索语法：`#标签`、`tag:标签`、`is:pinned`、`is:archived`、`is:active`、`has:link`、`has:web`、`has:ocr`、`has:image-edit`、`has:audio`、`has:video`、`has:scrapbook`、`has:worklog`、`has:wardrobe`、`has:outfit`、`created:2026-06`
...
- 统一素材索引 v1：现有记录会同步索引为文本、链接、网页摘录、图片文字、图片编辑、附件、音频、视频、任务、内部引用、手帐页面、工作日志、衣橱单品和穿搭素材，为后续自由裁剪、抠图和衣橱统计打底
...
图片编辑的“授权清理”只用于用户自己拍摄、拥有或已经获得授权的图片瑕疵修复，例如灰尘、污点、小杂物或排版遮盖；项目不把移除第三方版权标识、平台水印或规避授权限制作为目标。
