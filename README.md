...
当前代码仍处在“随记/素材库底座”阶段：已经完成文字记录、标签、搜索、附件、拍照/拍视频/相册/文件导入、录音 v1、本地语音转写 v1、视频缩略图缓存与媒体元数据摘要 v1、网页摘录 v3、网页/OCR 摘录片段索引 v1、图片/截图 OCR v2、素材索引、素材库视图、手帐页面与画布编辑 v5、工作日志 v5、图片编辑 v7、电子衣橱与穿着洞察 v7、分享扩展、引用、备份、隐私锁和可选 AI 洞察；真实天气 API 和单实例主体选择等仍在路线图中。
...
- 后续补连续扫描增强、转写语言选择、批量媒体元数据刷新和长列表性能验证
...
- 视频缩略图与媒体摘要 v1：详情附件区和素材库视频条目会在本机生成并缓存视频画面预览，附件行展示时长、分辨率和文件大小摘要，失败时回退为视频图标
- 网页摘录入口 v3：输入卡片识别到链接后可提取标题、摘要、来源、摘录卡和正文重点候选；单链接可合并草稿里的 OCR 文本并勾选摘录片段后保存，多链接会批量抓取并分别保存为多条网页摘录记录；保存后的片段会作为“摘录”素材进入素材库与搜索
- 图片/截图 OCR v2：拍摄或导入图片后用 Apple Vision 在本机识别文字，识别结果随附件保存为“图片文字”记录；详情页可对图片附件框选局部区域并追加识别文字，识别结果继续进入“图片文字”素材和 `has:ocr` 搜索
- 图片编辑 v7：素材库图片可打开编辑器，支持预设比例裁剪、拖拽定位/捏合缩放裁剪、背景柔化/纯色画布、人物抠图、iOS 17+ 智能主体抠图、用户拥有或获授权图片的瑕疵清理贴片、滤镜、边框、文字、贴纸和版式模板/导出预设；裁剪预览与导出渲染共用同一套边界规则，保存为新的本地 PNG 附件与“图片编辑”素材，并记录所用模板便于复现
- 电子手帐 v5：首页新增“手帐”模式，可保存手帐页面结构化记录、生成可迁移图层 JSON，在手帐列表显示页面预览，可在编辑器拖拽、缩放、旋转图层，追加已有图片素材，调整画布底色、字体、文字色、贴纸、底色、花边颜色、字号、圆角和线宽，把当前画布导出为 PNG 或 PDF 附件，并可选择多张图片快速生成图片拼贴和 PNG 导出附件
- 工作日志 v5：首页“日志”模式可按标签、素材类型、时间和关键词筛选可汇总记录，勾选已有记录，提取已完成/未完成任务，填写项目与日期范围，套用日报、周报、项目汇报或复盘模板，生成带来源引用的结构化工作记录；工作日志列表可按项目/模板/日期继续筛选，并把当前结果导出为 Markdown 后通过系统分享表发送，导出文件顶部会自动生成本地汇报摘要
- 电子衣橱 v7：首页新增“衣橱”模式，可快速保存单品、材质、厚薄、价格、穿搭组合、穿着记录、洗护状态和旅行打包清单，打包清单可记录行程天数、目的地和天气，单品可绑定已有图片附件，并进入衣橱/穿搭/穿着/洗护/打包素材索引；衣橱洞察会统计分类、颜色、季节、场景、未搭配单品、常用单品、最近穿着和成本/次，根据最近穿着或打包清单天气生成天气穿搭，炎热天气优先轻薄、透气材质，按行程天数扩展打包建议数量，优先带出最近打包清单的目的地和天气，避开待清洗/送洗/待熨烫/待修补单品，提醒洗护状态，可把穿搭建议或自动打包建议填入对应草稿，并可为待处理洗护单品安排本地系统通知
- `#标签` 自动识别
- 支持 `#项目/子类` 多级标签
- 标签筛选
- SQLite FTS 全文搜索
- 搜索语法：`#标签`、`tag:标签`、`is:pinned`、`is:archived`、`is:active`、`has:link`、`has:web`、`has:clip`、`has:ocr`、`has:image-edit`、`has:audio`、`has:video`、`has:scrapbook`、`has:worklog`、`has:wardrobe`、`has:outfit`、`has:wear-log`、`has:laundry-log`、`has:packing-list`、`created:2026-06`
...
- 统一素材索引 v1：现有记录会同步索引为文本、链接、网页摘录、摘录片段、图片文字、图片编辑、附件、音频、视频、任务、内部引用、手帐页面、工作日志、衣橱单品、穿搭、穿着记录、洗护记录和打包清单素材，为后续真实天气推荐、单实例主体选择和衣橱统计打底
- 素材库 v1：首页可按文本、链接、网页、摘录、图片文字、图片编辑、附件、音频、视频、任务、引用、手帐、日志、衣橱、穿搭等类型筛选素材，图片与视频附件支持本地预览，网页素材可直接打开原链接，图片素材可进入编辑器
