...
阶段 55：小组件快照与深链入口，状态：complete

备注：阶段 51 为完整备份导入反馈补充记录/历史/附件/大小摘要；阶段 52 为手帐图片图层补照片滤镜、相框颜色/线宽，并修复滤镜 renderer 的 CI 编译命名冲突；阶段 53 补 `.somebackup` 文件导入后的摘要反馈，并收拢远端日期解析测试的时区稳定性修复；阶段 54 回补 flomo 对标缺口中的禅定/专注记录入口；阶段 55 补小组件快照、最近记录和 home/zen/open 深链入口。
