...
- 如果保留工作日志 AI 润色，需要在隐私政策和审核说明中写明用户主动触发时会把工作汇报草稿发送给 OpenAI API。
...
- 如果保留衣橱天气查询，需要在隐私政策和审核说明中写明用户主动点击“获取天气”时会把填写的目的地发送给 Open-Meteo 地理编码和天气接口，不请求持续定位。
...
- 快捷指令保存随记，并能打开专注、工作日志、衣橱和 AI 整理页。
- 衣橱打包页填写目的地后点击“获取天气”，确认天气和备注填入草稿；网络失败时只显示错误，不影响保存手填打包清单。
- 隐私锁和每日提醒。
