...
当前明显缺口：已有 `MemoAsset` 索引表作为素材入口，已补图片 OCR、音频本机转写、视频按需缩略图、图片编辑 v1 和衣橱洞察 v1；但还没有独立媒体资产表、网页摘录实体表、自由裁剪/抠图管线、音视频长内容处理、穿着日历/成本/天气推荐模型。这些缺口已提升为下一轮优先级。
...
- 电子衣橱 MVP：单品和穿搭记录入口、分类/颜色/季节/场景统计、未搭配/常用单品、搭配建议草稿已完成 v1；穿着日历、成本/次、洗护状态、旅行打包、天气推荐和灵感板仍待补
...
- 图片编辑 MVP：素材库图片编辑入口、预设比例裁剪、Core Image 滤镜、边框、文字、贴纸和编辑作品素材索引已完成 v1；自由裁剪、抠图/背景移除、对象清理、模板拼贴和批量导出作为下一阶段
- 电子衣橱 MVP：单品建档、分类、颜色/季节/场景标签、搭配组合和使用洞察已完成 v1；穿着日历、成本/次和天气推荐继续排期
