...
- 电子手帐 MVP 第一段：首页新增“手帐”模式，可保存手帐页面结构化记录；记录会进入 `scrapbookPage` 素材索引，并支持 `has:scrapbook` / `has:手帐` 搜索筛选
...
- 多模态采集 MVP：相机拍照、录音、相册图片/视频导入、文件导入和图片 OCR 已完成 v1；视频拍摄、语音转写、批量元数据和缩略图缓存仍待补
...
- 电子手帐 MVP：页面结构化记录已完成第一段；真正的页面模型、素材拖拽、图片/文字/贴纸/边框/背景、字体与装饰编辑、导出图片/PDF 仍待补
