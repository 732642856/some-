...
- 多模态采集入口 v1：输入卡片新增拍照、相册图片/视频导入和文件导入，导入内容会保存到本地附件目录并生成附件 memo
...
- 电子衣橱 MVP 第一段：首页新增“衣橱”模式，可保存衣橱单品和穿搭组合；记录会进入 `wardrobeItem` / `outfit` 素材索引，并支持 `has:wardrobe` / `has:outfit` 搜索筛选
...
- 多模态采集 MVP：相机拍照、相册图片/视频导入、文件导入和图片 OCR 已完成 v1；录音、视频拍摄、批量元数据和缩略图缓存仍待补
