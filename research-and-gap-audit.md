...
- 电子手帐 MVP 第一段：首页新增“手帐”模式，可保存手帐页面结构化记录和可迁移图层 JSON；记录会进入 `scrapbookPage` 素材索引，并支持 `has:scrapbook` / `has:手帐` 搜索筛选，手帐列表会展示页面预览
...
- 多模态采集 MVP：相机拍照、视频拍摄、录音、相册图片/视频导入、文件导入和图片 OCR 已完成 v1；语音转写、批量元数据和缩略图缓存仍待补
