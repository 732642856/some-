...
下一轮开发建议做搜索二期或历史版本，因为：

- Share Extension 附件 v1 已完成，主动网页标题抓取可后置到联网能力更明确时。
- 搜索二期能提升已存内容的找回效率，是 flomo/memos 类产品的核心留存体验。
- 历史版本可以和 SQLite 本地存储自然衔接，降低误删/误改风险。
- 两者都不依赖签名证书，适合在当前环境继续推进。

推荐路线：

1. 搜索二期：命中高亮、最近搜索、保存筛选条件。
2. 历史版本：编辑前保存旧正文，详情页可查看和恢复。
3. 附件后续：如需按年月拆分 Markdown 和附件目录，再引入 ZIPFoundation 做 zip 包导出。
4. 网页后续：在明确网络策略后再做标题抓取。
